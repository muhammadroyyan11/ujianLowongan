...
class Master_model extends CI_Model
{
    ...
    {
        $this->datatables->select('id_bagian, nama_bagian, isActive');
        $this->datatables->from('bagian');
        $this->datatables->add_column('bulk_select', '<div class="text-center"><input type="checkbox" class="check" name="checked[]" value="$1"/></div>', 'id_bagian, nama_bagian, isActive');
        return $this->datatables->generate();
    }
    ...
    {
        $this->datatables->select('a.id_dosen,a.nip, a.nama_dosen, a.email, (SELECT COUNT(id) FROM users WHERE username = a.nip OR email = a.email) AS ada');
        $this->datatables->from('dosen a');
        return $this->datatables->generate();
    }

    public function getDataDosenn()
    {
        $this->db->select('a.id_dosen,a.nip, a.nama_dosen, a.email, (SELECT COUNT(id) FROM users WHERE username = a.nip OR email = a.email) AS ada');
